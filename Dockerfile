...
# Run migrations + seed, clear config cache (so CORS is fresh), then serve
CMD sh -c 'php artisan migrate --force && php artisan db:seed --force && php artisan config:clear && php artisan serve --host=0.0.0.0 --port=${PORT:-8000}'
